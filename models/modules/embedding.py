...
class AtomEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): The atom features to be encoded. 
            The shape is (N, 9) or (B, N, 9), where B is a batch size and N is the number of atoms.
        ...
        if len(x.shape) == 2:
            ...
            dim = 2
            indices_ls = torch.split(x, 1, dim=2)
        embedding_ls = [self.embedding_list[i](indices_ls[i]) for i in range(len(indices_ls))]

        if self.attr_reduction == "mean":
            return torch.mean(torch.cat(embedding_ls, dim=dim), dim=dim)
        elif self.attr_reduction == "sum":
            return torch.sum(torch.cat(embedding_ls, dim=dim), dim=dim)
...
class BondEmbedding(nn.Module):
    ...
    def forward(self, edge_attr: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            edge_attr (torch.Tensor): The bond features to be encoded. The shape is (E, 4), E is the number of bonds.
